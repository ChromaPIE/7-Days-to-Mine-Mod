	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (raytraceresult == null) {
			...
			if (flag) {
				...
				EntityCar minibike = new EntityCar(worldIn, raytraceresult.hitVec.x,
						flag1 ? raytraceresult.hitVec.y - 0.12D : raytraceresult.hitVec.y, raytraceresult.hitVec.z);
				minibike.rotationYaw = playerIn.rotationYaw;

				if (!worldIn.getCollisionBoxes(minibike, minibike.getEntityBoundingBox().grow(-0.1D)).isEmpty()) {
					return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
				} else {
					if (!worldIn.isRemote) {
						worldIn.spawnEntity(minibike);
					}
