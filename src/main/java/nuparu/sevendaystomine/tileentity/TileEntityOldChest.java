...
public class TileEntityOldChest extends TileEntityLockableLoot implements ITickable
{
    ...
    public TileEntityOldChest adjacentChestZNeg;
    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityOldChest adjacentChestXPos;
    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityOldChest adjacentChestXNeg;
    /** Contains the chest tile located adjacent to this one (if any) */
    public TileEntityOldChest adjacentChestZPos;
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityOldChest.class, new String[] {"Items"}));
    }
    ...
    private void setNeighbor(TileEntityOldChest chestTe, EnumFacing side)
    {
    ...
    @Nullable
    protected TileEntityOldChest getAdjacentChest(EnumFacing side)
    {
        ...
        if (this.isChestAt(blockpos))
        {
            ...
            {
            	TileEntityOldChest tileentitychest = (TileEntityOldChest)tileentity;
    ...
    private boolean isChestAt(BlockPos posIn)
    {
        ...
        else
        {
            ...
            return block instanceof BlockChestOld;
        }
    }
    ...
    {
        if (!player.isSpectator() && this.getBlockType() instanceof BlockChestOld)
        {
            --this.numPlayersUsing;
            this.world.addBlockEvent(this.pos, this.getBlockType(), 1, this.numPlayersUsing);
            this.world.notifyNeighborsOfStateChange(this.pos, this.getBlockType(), false);
	...
	public net.minecraft.util.math.AxisAlignedBB getRenderBoundingBox() {
		return new AxisAlignedBB(getPos().add(-2, -2, -2), getPos().add(2, 2, 2));
	}
}
