...
public class WorldGenBigTreeBurnt extends WorldGenAbstractTree {
	private Random rand;
	private World world;
	...
	List<WorldGenBigTreeBurnt.FoliageCoordinates> foliageCoords;

	public WorldGenBigTreeBurnt(boolean notify) {
		super(notify);
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenBigTreeBurnt.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenBigTreeBurnt.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords
									.add(new WorldGenBigTreeBurnt.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_) {
		...
		for (int j = 0; j <= i; ++j) {
			...
			EnumFacing.Axis blocklog$enumaxis = this.getLogAxis(p_175937_1_, blockpos1);
			this.setBlockAndNotifyAdequately(this.world, blockpos1,
					p_175937_3_.getDefaultState().withProperty(BlockPillarBase.AXIS, blocklog$enumaxis));
		}
	}
	...
	private EnumFacing.Axis getLogAxis(BlockPos p_175938_1_, BlockPos p_175938_2_) {
		EnumFacing.Axis blocklog$enumaxis = EnumFacing.Axis.Y;
		...
		if (k > 0) {
			if (i == k) {
				blocklog$enumaxis = EnumFacing.Axis.X;
			} else if (j == k) {
				blocklog$enumaxis = EnumFacing.Axis.Z;
			}
		}
	...
	void generateTrunk() {
		...
		Block block = ModBlocks.BURNT_LOG;
	...
	void generateLeafNodeBases() {
		for (WorldGenBigTreeBurnt.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
			...
			if (!blockpos.equals(worldgenbigtree$foliagecoordinates)
				...
				this.limb(blockpos, worldgenbigtree$foliagecoordinates, ModBlocks.BURNT_LOG);
			}
		}
	}
	...
	private boolean validTreeLocation() {
		...
		boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP,
				((net.minecraft.block.BlockSapling) Blocks.SAPLING)) || state.getBlock() instanceof BlockDryGround;
