 ...
 *  things like WoodlandMansion, but overrides most if not all of the code
 */
public class ChunkGeneratorWasteland extends ChunkGeneratorOverworld {
	...
	private MapGenScatteredFeature scatteredFeatureGenerator = new MapGenWastelandScatteredFeature();
	...
	public ChunkGeneratorWasteland(World worldIn, long seed, boolean mapFeaturesEnabledIn, String generatorOptions) {
		super(worldIn, seed, mapFeaturesEnabledIn, generatorOptions);
		{
			...
			woodlandMansionGenerator = (WoodlandMansion) net.minecraftforge.event.terraingen.TerrainGen.getModdedMapGen(
	...
	public Chunk generateChunk(int x, int z) {
		...
		if (this.mapFeaturesEnabled) {
			...
			/*
			 * if (this.settings.useVillages) { this.villageGenerator.generate(this.world,
			 * x, z, chunkprimer); }
			 */
	...
	private void generateHeightmap(int x, int p_185978_2_, int z) {
		this.depthRegion = this.depthNoise.generateNoiseOctaves(this.depthRegion, x, z, 5, 5,
		...
		this.mainNoiseRegion = this.mainPerlinNoise.generateNoiseOctaves(this.mainNoiseRegion, x, p_185978_2_, z, 5, 33,
				5, (double) (f / this.settings.mainNoiseScaleX), (double) (f1 / this.settings.mainNoiseScaleY),
				(double) (f / this.settings.mainNoiseScaleZ));
		this.minLimitRegion = this.minLimitPerlinNoise.generateNoiseOctaves(this.minLimitRegion, x, p_185978_2_, z, 5,
				33, 5, (double) f, (double) f1, (double) f);
		this.maxLimitRegion = this.maxLimitPerlinNoise.generateNoiseOctaves(this.maxLimitRegion, x, p_185978_2_, z, 5,
				33, 5, (double) f, (double) f1, (double) f);
		int i = 0;
		int j = 0;
	...
	public void populate(int x, int z) {
		...
		if (this.mapFeaturesEnabled) {
			...
			/*
			 * if (this.settings.useVillages) { flag =
		...
		if (biome != ModBiomes.WASTELAND_DESERT && biome != Biomes.DESERT && biome != Biomes.DESERT_HILLS
