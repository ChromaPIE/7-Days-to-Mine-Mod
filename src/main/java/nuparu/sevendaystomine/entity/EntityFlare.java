	...
	public void onUpdate() {
		...
		if (++age >= 600) {
			if (!world.isRemote) {
				...
				world.spawnEntity(e);
				int x = (int) (posX + rand.nextInt(5) - 2);
				int z = (int) (posZ + rand.nextInt(5) - 2);
				e.setPosition(x, 255, z);
				server.getPlayerList().sendMessage(new TextComponentTranslation("airdrop.message",
						x + MathUtils.getIntInRange(world.rand, 32, 128) * (world.rand.nextBoolean() ? 1 : -1),
						z + MathUtils.getIntInRange(world.rand, 32, 128) * (world.rand.nextBoolean() ? 1 : -1)));
			}
			this.setDead();
			return;
		}
			...
			for (int i1 = 0; i1 < 5; ++i1) {
				...
				if (isInWater()) {
					...
					this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL,
							this.posX + (this.rand.nextDouble() * 0.2 - 0.1) * (double) this.width,
							this.posY + this.rand.nextDouble() * (double) this.height,
							this.posZ + (this.rand.nextDouble() * 0.2 - 0.1) * (double) this.width,
							(this.rand.nextDouble() * 0.2 - 0.1), 0.4 + (this.rand.nextDouble() * 0.2 - 0.1),
							(this.rand.nextDouble() * 0.2 - 0.1), new int[0]);
				}
			}
		}
