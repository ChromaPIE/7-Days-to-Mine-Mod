...
public class EntityChlorineGrenade extends Entity implements IProjectile {
	...
	public EntityChlorineGrenade(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public EntityChlorineGrenade(World worldIn, EntityLivingBase throwerIn) {
		this(worldIn, throwerIn.posX, throwerIn.posY + (double) throwerIn.getEyeHeight() - 0.10000000149011612D,
				throwerIn.posZ);
		this.thrower = throwerIn;
	}

	public EntityChlorineGrenade(World worldIn, double x, double y, double z) {
		this(worldIn);
	...
	public void onUpdate() {
		...
		if (age > 120) {
			if (this.age == 121) {
				this.world.playSound(null, posX, posY, posZ, SoundHelper.GAS_LEAK, SoundCategory.AMBIENT,
			...
			for (int i1 = 0; i1 < 5; ++i1) {
				...
				if (isInWater()) {
					this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE,
							this.posX + (this.rand.nextDouble() * 0.2 + 0.4) * (double) this.width,
							this.posY + this.rand.nextDouble() * (double) this.height,
							this.posZ + (this.rand.nextDouble() * 0.2 + 0.4) * (double) this.width,
							rand.nextFloat() - 0.5, rand.nextFloat() + 0.3, rand.nextFloat() - 0.5, new int[0]);
				} else {
					int c = 1930808;
					...
					this.world.spawnParticle(EnumParticleTypes.SPELL_MOB,
							this.posX + (this.rand.nextDouble() * 0.2 + 0.4) * (double) this.width,
							this.posY + this.rand.nextDouble() * (double) this.height,
							this.posZ + (this.rand.nextDouble() * 0.2 + 0.4) * (double) this.width, d3, d4, d5,
							new int[0]);
				}
			}
		}
	...
	private void applySplash() {
		...
		if (!list.isEmpty()) {
			for (EntityLivingBase entitylivingbase : list) {
				if (entitylivingbase.canBeHitWithPotion() && entitylivingbase.isInWater() == isInWater()) {
					double d0 = this.getDistanceSq(entitylivingbase);
					double d1 = 1.0D - Math.sqrt(d0) / 4.0D;
					if (d0 < 16.0D) {
						int i = (int) (d1 * (double) 1200 + 0.5D);
						entitylivingbase.addPotionEffect(new PotionEffect(Potions.chlorinePoison, i, 0, false, false));
					}
				}
			}
		}
