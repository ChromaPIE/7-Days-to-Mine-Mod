...
public class TileEntityStreetSignRenderer extends TileEntitySpecialRenderer<TileEntityStreetSign> {
	private static final ResourceLocation SIGN_TEXTURE = new ResourceLocation(SevenDaysToMine.MODID,
			"textures/entity/street_sign.png");
	/** The ModelSign instance for use in this renderer */
	private final ModelSign model = new ModelSign();

	public void render(TileEntityStreetSign te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		Block block = te.getBlockType();
		GlStateManager.pushMatrix();
		float f = 0.6666667F;

		if (block == ModBlocks.STREET_SIGN) {
