...
public class CureTrigger implements ICriterionTrigger {
	private final ResourceLocation RL;
	private final Map listeners = Maps.newHashMap();

	public CureTrigger(String parString) {
		super();
		RL = new ResourceLocation(parString);
	}

	public CureTrigger(ResourceLocation parRL) {
		super();
		RL = parRL;
	}
	...
	public void addListener(PlayerAdvancements playerAdvancementsIn, ICriterionTrigger.Listener listener) {
		CureTrigger.Listeners myCustomTrigger$listeners = (Listeners) listeners.get(playerAdvancementsIn);

		if (myCustomTrigger$listeners == null) {
			myCustomTrigger$listeners = new CureTrigger.Listeners(playerAdvancementsIn);
	...
	public void removeListener(PlayerAdvancements playerAdvancementsIn, ICriterionTrigger.Listener listener) {
		CureTrigger.Listeners tameanimaltrigger$listeners = (Listeners) listeners.get(playerAdvancementsIn);
	 ...
	 */
	@Override
	public CureTrigger.Instance deserializeInstance(JsonObject json, JsonDeserializationContext context) {
		return new CureTrigger.Instance(getId());
	}
	...
	public void trigger(EntityPlayerMP parPlayer) {
		CureTrigger.Listeners tameanimaltrigger$listeners = (Listeners) listeners.get(parPlayer.getAdvancements());
