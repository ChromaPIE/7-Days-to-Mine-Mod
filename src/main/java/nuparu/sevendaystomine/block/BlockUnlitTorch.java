...
public class BlockUnlitTorch extends BlockBase {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!itemstack.isEmpty() && (item == Items.FLINT_AND_STEEL || item == Item.getItemFromBlock(Blocks.TORCH)
				|| item == Item.getItemFromBlock(nuparu.sevendaystomine.init.ModBlocks.TORCH_LIT))) {
			worldIn.setBlockState(pos, nuparu.sevendaystomine.init.ModBlocks.TORCH_LIT.getDefaultState()
					.withProperty(BlockTorch.FACING, state.getValue(FACING)), 3);
